 ...
 */
function bpbs_enqueue_custom_admin_style() {
    wp_register_style( 'bpbs_admin_custom_css', plugin_dir_url(__FILE__) . 'style.css', false, '1.0.0' );
    wp_enqueue_style( 'bpbs_admin_custom_css' );
}
add_action( 'admin_enqueue_scripts', 'bpbs_enqueue_custom_admin_style' );


//wp_enqueue_style('bpbs_admin-style', plugin_dir_url(__FILE__) . 'admin/css/style.css');
...
add_action('admin_menu', 'bpbs_support_page');

function bpbs_support_page()
{
    add_submenu_page('edit.php?post_type=bpbs-showcase', 'Help ', 'Help', 'manage_options', 'bpbs-support', 'bpbs_support_page_callback');
}

function bpbs_support_page_callback()
{
    ?>
                    ...
                    <div class="bplugins-feature center">
                        <div style="background:white;overflow:hidden;">
                            <div style="width:128px;heigh:128px;overflow:hidden;float:left;">
                                <img src="https://ps.w.org/showcase-it/assets/icon-128x128.png?rev=2570335" alt="Logo" width="100" height="100">
                            </div>
                            <div style="float:left; overflow:hidden;text-align:left;">
                                <h1><?php echo esc_html__('Thanks for Installing Showcase-It Plugin.', 'model-viewer'); ?></h1>
...
// Pro Plugin List
//------------------------------------------------------------------
add_action('admin_menu', 'bpbs_pro_plugin_page');

function bpbs_pro_plugin_page() {
	add_submenu_page( 'edit.php?post_type=videoplayer', 'Our PRO Plugins', 'Our PRO Plugins', 'manage_options', 'bpbs-pro-plugins', 'bpbs_proplugin_page_cb' );
}

function bpbs_proplugin_page_cb() {
...
function bpbs_free_plugin_loaded(){
    wp_enqueue_script('plugin-install');
    wp_enqueue_script('updates');
}
add_action('init', 'bpbs_free_plugin_loaded');
//$table->display();
                ...
                <div class="plugin-card plugin-card-<?php echo $plugin['slug']; ?>">
                    <div class="plugin-card-top">
                        <div class="name column-name">
                            <h3><a href="<?php echo $details_link; ?>" class="thickbox open-plugin-details-modal"><?php echo $plugin['name']; ?> <img src="<?php echo $plugin['icons']['1x'] ?>"  class="plugin-icon" alt="">
                         </a>
                            </h3>
                        </div>
                        ...
                        <ul class="plugin-action-buttons">
                            <li>
                                <?php 
                                    if ($action_links) {echo implode("</li><li>", $action_links); }
                                ?>
                            </li>
                        </ul>
                        </div>
